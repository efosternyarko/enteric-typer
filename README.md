...
## assemble.nf — Standalone ONT Assembly Pipeline

`assemble.nf` is a separate Nextflow workflow for assembling Oxford Nanopore reads
into genome assemblies. It is independent of `main.nf` and produces assembled FASTAs
that can be fed directly into enteric-typer.

### Workflow steps

1. **Read QC** — `seqkit stats` on each sample → per-sample metrics TSV (read count,
   total bases, read N50, mean length, mean quality, GC%, estimated depth)
2. **Depth filter** — samples below `--min_read_depth` (default 20×) are dropped with
   a warning and excluded from the plot and assembly
3. **Read metrics plot** — 8-panel figure (histograms + boxplots) for passing samples:
   read N50, estimated depth, total reads, mean read length
4. **Assembly** — one of five assemblers (hybracter by default)

### Assemblers

| `--assembler` | Tool | Notes |
|---|---|---|
| `hybracter` | [Hybracter](https://github.com/gbouras13/hybracter) | Default. Circularises chromosomes and plasmids. Uses `--auto` to estimate chromosome size. |
...
| `unicycler` | [Unicycler](https://github.com/rrwick/Unicycler) | Long-read only mode (`--long`). Produces circularised assemblies where possible. |
...
# macOS Apple Silicon — add arm64 profile and skip medaka (OpenSSL conflict)
...
# HPC (SLURM)
nextflow run assemble.nf -c assemble.config -profile conda,slurm \
    --input_dir /path/to/fastq/
```

**Feed assembled FASTAs into enteric-typer:**

When `--assembler hybracter` (the default), assembled FASTAs are collected into
`assembly_results/all_assemblies/`. Pass this directory directly to enteric-typer:
...
| Parameter | Default | Description |
|---|---|---|
| `--input_dir` | `null` | Directory of FASTQ / FASTQ.gz files (sample ID = filename stem) |
...
| `--min_read_depth` | `20` | Minimum estimated depth (×) — shallower samples are skipped |
...
| `--hybracter_no_medaka` | `false` | Skip medaka polishing. **Required on macOS Apple Silicon** (OpenSSL dependency conflict). Leave `false` on Linux/HPC. |
...
│         Per-sample read metrics (one row, tab-separated):
...
│   │     8-panel figure (passing samples only, i.e. depth ≥ --min_read_depth):
│   │       A  Read N50 histogram   B  Read N50 boxplot
│   │       C  Depth histogram      D  Depth boxplot
│   │           (vertical dashed line marks --min_read_depth)
...
│
│  ── Assembly (hybracter, default) ────────────────────────────────────────
│
├── all_assemblies/                        ← feed this into enteric-typer
│   └── {sample}.fasta
│         Chromosome + plasmid sequences merged into a single FASTA.
│         For complete assemblies: chromosome sequence(s) followed by any
│         plasmid contigs. For incomplete assemblies: draft contig set.
│         Each FASTA header retains the original hybracter sequence name.
│
├── hybracter_output/
...
│   │     Draft contig set for samples where the chromosome could not be
│   │     circularised (present only for incomplete assemblies).
│   └── {sample}_hybracter_summary.tsv
│         One-row TSV from hybracter summarising assembly outcome:
│           sample            — sample name
...
│           (additional columns vary by hybracter version)
│
│  ── Assembly (flye / dragonflye / unicycler) ─────────────────────────────
│
├── assemblies/
│   └── {sample}.fasta        ← assembled contigs (feed into enteric-typer)
│
...
│
└── pipeline_info/
    ├── timeline.html   — per-task runtime and CPU/memory usage
    ├── report.html     — summary execution report
    └── dag.svg         — workflow directed acyclic graph
```

> **Hybracter: complete vs incomplete assemblies.**
> Hybracter classifies each assembly as *complete* (chromosome circularised) or
> *incomplete* (linear draft contigs). The `{sample}_hybracter_summary.tsv` file
> reports this for every sample. For complete assemblies, the chromosome FASTA and
> (if present) plasmid FASTA are both kept separately in `hybracter_output/` and
> merged into the single `all_assemblies/{sample}.fasta`. For incomplete assemblies,
> only the draft contig FASTA (`{sample}_incomplete.fasta`) is retained.
...
- **Autocycler**: github.com/rrwick/Autocycler
